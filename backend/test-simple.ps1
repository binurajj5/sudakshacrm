...
Write-Host "`nCreating a new course..." -ForegroundColor Cyan
try {
    $headers = @{
...
Write-Host "`nListing all courses..." -ForegroundColor Cyan
try {
    ...
    Write-Host "   Page: $($courses.page)" -ForegroundColor Gray
} catch {
    Write-Host "Listing courses failed: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host "`nRetrieving course details..." -ForegroundColor Cyan
try {
